...
from xlin import append_to_json_list, generate_classification_report, convert_to_jsonable_report
import uuid
import datetime
import pandas as pd
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        self._save_log_rows(total_log_rows, "validation", f"{len(total_log_rows)}_{str(uuid.uuid4())}")
        ...
        reporct_metric = self._save_report(total_log_rows, "test")
        metric_dict.update(reporct_metric)
        splits = list(set([row["split"] for row in total_log_rows if "split" in row]))
        for split in splits:
            split_metric = self._save_report([row for row in total_log_rows if row["split"] == split],
                                             f"test-{split}")
            metric_dict.update(split_metric)
        return metric_dict
    ...
    def _save_log_rows(self, log_rows: list[dict], split: str, info: str):
        actor_local_path = os.path.join(
            self.config.trainer.default_local_dir,
            "logs",
            f"{split}_step{self.global_steps}_{info}.jsonl",
        )
        append_to_json_list(log_rows, actor_local_path)

    def _save_report(self, log_rows: list[dict], split: str):
        actor_local_path = os.path.join(
            self.config.trainer.default_local_dir,
            "logs",
            f"{split}_reports.jsonl",
        )
        preds, labels = [], []
        ...
        report_row["global_step"] = self.global_steps
        report_row["split"] = split
        if len(log_rows) > 0:
            for k in log_rows[0]:
        ...
        append_to_json_list([jsonable_report_row], actor_local_path)
        class_report = jsonable_report_row["class_report"]
        micro_stats = jsonable_report_row["micro_stats"]
        macro_stats = jsonable_report_row["macro_stats"]
        metric = {}
        for v in class_report:
            class_name = v["class"]
            for metric_name in v:
                if metric_name == "class":
                    continue
                metric[f"{split}/{class_name}/{metric_name}"] = v[metric_name]
        for k, v in micro_stats.items():
            metric[f"{split}/micro/{k}"] = v
        for k, v in macro_stats.items():
            metric[f"{split}/macro/{k}"] = v
        top_names = ["precision", "recall", "f1"]
        new_metric = {}
        for name in top_names:
            for k in metric:
                if name in k:
                    new_metric[k] = metric[k]
        new_metric.update(metric)
        metric = new_metric
        for k in report_row:
            if k in ["confusion_matrix", "class_report", "response_length", "format_correct", "answer_correct"] or "score" in k:
                value = report_row[k]
                if isinstance(value, pd.DataFrame):
                    if value.index.name is not None:
                        value = value.reset_index()
                metric[f"{split}/{k}"] = value
        return metric
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_extra_infos_dict, log_rows = compute_reward(batch, self.reward_fn)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            reward_tensor, reward_extra_infos_dict, log_rows = ray.get(future_reward)
                        batch.batch["token_level_scores"] = reward_tensor

                        if log_rows:
                            self._save_log_rows(log_rows, "train", f"epoch{epoch}_{len(log_rows)}_{str(uuid.uuid4())}")
                            # TODO: profile here 根据结果调整reward
                            report_metric = self._save_report(log_rows, "train")
                            metrics.update(report_metric)
