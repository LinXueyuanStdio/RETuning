...
def process_validation_metrics(data_sources: list[str], sample_inputs: list[str], infos_dict: dict[str, list[Any]], seed: int = 42) -> dict[str, dict[str, dict[str, float]]]:
    ...
    for sample_idx, data_source in enumerate(data_sources):
        ...
        data_source_parts = []
        if "," in data_source:
            data_source_parts = data_source.split(",")
            data_source_list.extend(data_source_parts)
        for part in data_source_parts:
            if "/" in part:
                data_source_split = part.split("/")
                if len(data_source_split) > 2:
                    for i in range(1, len(data_source_split)-1):
                        parent_data_source = "/".join(data_source_split[:i+1])
                        data_source_list.append(parent_data_source)
        data_source_list = sorted(list(set(data_source_list)), key=lambda x: len(x.split("/")))
        ...
        for data_source in data_source_list:
            ...
            for var_name, var_vals in infos_dict.items():
                ...
                if var_name == "reward":
                    if var_vals[sample_idx] >= 3:
                        var2vals["recall"].append(1)
                    else:
                        var2vals["recall"].append(0)
